...
class Setup extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        if (!$this->editDatabaseProps(array($db_hostname, $db_name, $db_user, $db_pswd, $db_driver, $db_prefix, $db_port))) {
        ...
        Database::forge()->dropDatabase($db_name);
    ...
    public function editDatabaseProps(array $props): bool {
        ...
        for ($i = 0; $i < sizeof($settings); $i++) {
            ...
            $result = file_put_contents($envFile, preg_replace($pattern, "{$settings[$i]} = {$props[$i]}", file_get_contents($envFile)));
